...
public class PidAutoCommand extends Command {
  ...
  private final double m_xPrime, m_zPrime, m_finalYa;
    ...
    public PidAutoCommand(DrivetrainSubsystem drivetrainSubsystem, ObjectTrackerSubsystem objectTrackerSubsystem, int tagID, double xPrime, double zPrime, double finalYa, boolean isVisionAuto, String yoloObject) {
    ...
    public PidAutoCommand(DrivetrainSubsystem dts, ObjectTrackerSubsystem ots, double xTarget, double yTarget, double rotTarget) {
      this(dts, ots, -1, 0, 0, 0, false, null); //Sets visionOffsets to 0 to not intefere with targetPose calcs
      m_x_target = xTarget;
      m_y_target = yTarget;
